...
public sealed class RiskAnalysisAgentTests
{
        ...
        IReadOnlyCollection<CommercialAnalysisMetricRuleSnapshot>? metricRules = null)
    {
        ...
        return new(
            new OpportunitySnapshot(Guid.NewGuid().ToString(), "Teste", pipelineId, stageId, null, null, 1000m, "active", false, now.AddDays(-20), now.AddDays(-20), null),
    ...
    private static RiskAnalysisAgent CreateAgent(IOpenAiRiskAnalysisClient openAiClient)
        => new(openAiClient, new RiskAnalysisAgentInputBuilder(new CommercialRuleAssessmentService()));
